...
/// while it is open, printable keys are text, not commands.
pub(crate) fn key_to_action(key: KeyEvent, jumping: bool) -> Option<Action> {
    ...
    match key.code {
        ...
        // reason the gpui frontend binds `=` for zoom-in.
...
mod tests {
    ...
    fn every_documented_binding_maps() {
        ...
        assert_eq!(
            key_to_action(key(KeyCode::BackTab), false),
            Some(Action::FocusPrev)
        );
        assert_eq!(
            key_to_action(key(KeyCode::Left), false),
        ...
        assert_eq!(
            key_to_action(key(KeyCode::Home), false),
            Some(Action::Move(Nav::Home))
        );
        assert_eq!(
            key_to_action(key(KeyCode::End), false),
            Some(Action::Move(Nav::End))
        );
        assert_eq!(
            key_to_action(key(KeyCode::Char('1')), false),
            Some(Action::SetColormap(Colormap::None))
        );
        assert_eq!(
            key_to_action(key(KeyCode::Char('4')), false),
            Some(Action::SetColormap(Colormap::Entropy))
        );
        assert_eq!(
            key_to_action(key(KeyCode::Char('y')), false),
            Some(Action::CopyHex)
        );
        assert_eq!(
            key_to_action(key(KeyCode::Char('Y')), false),
        ...
        assert_eq!(
            key_to_action(key(KeyCode::Char('g')), false),
        ...
        assert_eq!(key_to_action(ctrl('c'), false), Some(Action::Quit));
    }

    #[test]
    fn shift_arrows_extend_rather_than_move() {
        assert_eq!(
            key_to_action(shift(KeyCode::Right), false),
            Some(Action::Extend(Nav::Right))
        );
        assert_eq!(
            key_to_action(shift(KeyCode::Down), false),
            Some(Action::Extend(Nav::Down))
        );
        assert_eq!(
            key_to_action(shift(KeyCode::PageDown), false),
    ...
    fn plus_is_reachable_without_shift() {
        assert_eq!(
            key_to_action(key(KeyCode::Char('+')), false),
    ...
    fn the_jump_prompt_is_modal() {
        ...
        assert_eq!(
            key_to_action(key(KeyCode::Char('1')), true),
        ...
        assert_eq!(
            key_to_action(key(KeyCode::Esc), true),
            Some(Action::JumpCancel)
        );
        assert_eq!(
            key_to_action(key(KeyCode::Backspace), true),
            Some(Action::JumpBackspace)
        );
        // Ctrl+C still quits mid-prompt.
        assert_eq!(key_to_action(ctrl('c'), true), Some(Action::Quit));
        // Arrows are not text, and are not commands here either.
        assert_eq!(key_to_action(key(KeyCode::Left), true), None);
    }

    #[test]
    fn unbound_keys_are_ignored() {
        assert_eq!(key_to_action(key(KeyCode::Char('z')), false), None);
        assert_eq!(key_to_action(key(KeyCode::F(5)), false), None);
        assert_eq!(key_to_action(key(KeyCode::Insert), false), None);
    }
}
